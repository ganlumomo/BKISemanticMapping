...
#include "stanford_semantics_util.h"

int main(int argc, char **argv) {
    ros::init(argc, argv, "2d3dsemantics_node");
    ros::NodeHandle nh("~");
    ...
    nh.param<std::string>("query_data_prefix", query_data_prefix, query_data_prefix);
    ...
    ROS_INFO_STREAM("Parameters:" << std::endl <<
      ...
      "query_data_prefix: " << query_data_prefix << std::endl << 
    ...
    ///////// Build Map /////////////////////
    StanfordSemanticsData stanford_semantics_data(nh, resolution, block_depth, sf2, ell, num_class, free_thresh, occupied_thresh, var_thresh, ds_resolution, free_resolution, max_range, map_topic, prior);
    stanford_semantics_data.read_lidar_poses(dir + '/' + lidar_pose_file);
    stanford_semantics_data.set_up_evaluation(dir + '/' + query_data_prefix, dir + '/' + evaluation_result_prefix);
    stanford_semantics_data.process_scans(dir + '/' + input_data_prefix, dir + '/' + input_label_prefix, scan_num, query, visualize);
